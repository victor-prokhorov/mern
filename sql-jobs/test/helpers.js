...
async function truncateAll() {
  await pool.query('TRUNCATE jobs, messages, accounts RESTART IDENTITY CASCADE')
}
...
export async function createAccount(overrides = {}) {
  const res = await httpAgent.post('/api/accounts').send({ name: 'acc', ...overrides })
  if (res.status !== 201) throw new Error(`createAccount(${JSON.stringify(overrides)}) got ${res.status}: ${JSON.stringify(res.body)}`)
  return res.body
}

export async function createMessage(overrides = {}) {
  const res = await httpAgent.post('/api/messages').send({ recipient: 'a@b.test', body: 'hi', ...overrides })
  if (res.status !== 201) throw new Error(`createMessage(${JSON.stringify(overrides)}) got ${res.status}: ${JSON.stringify(res.body)}`)
  return res.body
}
