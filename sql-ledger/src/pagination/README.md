...
`src/pagination/cursor.js` is the only place that touches the cursor's wire format: `encodeCursor` (`cursor.js:3-6`) JSON-encodes `{ c: createdAt, i: String(id) }` and base64url-encodes the result; `decodeCursor` (`cursor.js:8-20`) reverses that and validates the shape before trusting it — a non-base64 string, valid base64 that isn't JSON, JSON missing either field, or a `c` that doesn't parse as a real date all throw `BadRequestError`, which the shared error handler turns into a 400. `id` is round-tripped as a **string**, not a number: Postgres returns `bigint` columns as strings over the wire specifically to avoid silent precision loss past `Number.MAX_SAFE_INTEGER`, and the cursor codec preserves that rather than coercing through a JS number at any point.

The `c` value is a string for the same reason, and a subtler one: it is **not** the `created_at` JS `Date` node-postgres hands back, but a full-precision text rendering of the same column (`to_char(created_at AT TIME ZONE 'UTC', ...)` aliased `created_at_cursor` in `findPageKeyset`), selected alongside the row purely to feed the cursor. `timestamptz` carries microseconds; a JS `Date` carries milliseconds. Round-tripping the parsed `Date` through the cursor would hand the next page a boundary up to 999µs *earlier* than the boundary row's true timestamp, so `(created_at, id) < ($1, $2)` would silently drop every remaining row sharing the boundary row's millisecond — reproduced in `test/pagination.test.js` with three transfers stamped `.123100`/`.123400`/`.123900` inside one millisecond, where the second page came back empty. Keeping the value as Postgres-rendered text end to end (`decodeCursor` validates it parses as a date but returns the string untouched; the predicate casts it back with `$1::timestamptz`) means the microseconds never pass through a JS `Date` at all. `sql-jobs`' queue README documents the same bug class for its fencing token, where it solved it the other way around — truncating the column to milliseconds with `date_trunc` instead of keeping the wire format lossless.
...
```json
{"transfers":[{"id":"105",...},{"id":"102",...}],"nextCursor":"eyJjIjoiMjAyNi0wOC0xNVQyMTo1MjoyNC42NTA0MjFaIiwiaSI6IjEwMCJ9"}
```

That cursor is the point of the "opaque" discussion above — `base64url --decode` it and you get `{"c":"...","i":"100"}` in plain text (note the six fractional digits in `c`: that is the full-precision text described in How it works, not a JS `Date`'s three). Now reproduce the `OFFSET` bug, which is the whole reason the other endpoint exists. Fetch page one by offset, insert a transfer, then fetch page two:
