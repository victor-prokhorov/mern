...
- **Database choice is decided by this workload's one non-negotiable: multi-row transactions with real isolation.** That means relational first — **RDS/Aurora Postgres** on AWS, **Cloud SQL/AlloyDB** on GCP, **Spanner** when you need the same guarantees across regions. DynamoDB and Firestore both have transactions (DynamoDB `TransactWriteItems` caps at 100 items; Firestore transactions are optimistic with retries), so a ledger *can* be built on them — but you give up `SERIALIZABLE`-class isolation options, deferred constraints, and ad-hoc SQL over the entries, and you re-implement the sum-to-zero invariant in application code, which is exactly what this README argues against. A document store with no transactions at all (this repo's standalone Mongo) is disqualified outright — which is the reason this app is the one that left the MERN stack.
- **The purpose-built option exists**: **TigerBeetle** (self-hosted, runs fine on either cloud) is a database whose only schema is accounts and transfers, with debit/credit invariants enforced natively at millions of transfers per second — the signal that ledger-shaped writes are a recognized workload class, not an exotic need.
- **Aurora and AlloyDB change operations, not semantics**: same Postgres wire protocol and isolation levels, so everything in this README — `FOR UPDATE`, deferred constraint triggers, `SERIALIZABLE` — transfers unchanged; what you buy is storage-level replication, failover, and read replicas. The one thing to re-check after any managed migration is the default isolation level and whether your driver's pooler (RDS Proxy, AlloyDB's connection pooling) preserves session state like advisory locks — many poolers in transaction mode silently break session-scoped features.
- **Reporting reads move off the primary**: balance dashboards and entry exports go to read replicas or get streamed (CDC, see the outbox README) into **Redshift/BigQuery** — the ledger's write path stays small and hot, analytics never contend with it.
