...
describe('check-layers', () => {
  it('catches a service function that calls res.status(...) directly', () => {
    const srcDir = mkdtempSync(path.join(os.tmpdir(), 'sql-scheduler-layers-'))
  ...
  it('catches SQL written outside repositories/', () => {
    const srcDir = mkdtempSync(path.join(os.tmpdir(), 'sql-scheduler-layers-'))
    const queueDir = path.join(srcDir, 'queue')
    mkdirSync(queueDir)
    writeFileSync(path.join(queueDir, 'violation.js'), "const sql = 'SELECT 1 FROM jobs'\n")

    const { sqlHits } = checkLayers(srcDir)

    rmSync(srcDir, { recursive: true, force: true })
    expect(sqlHits.some((hit) => hit.includes('SELECT 1 FROM jobs'))).to.equal(true)
  })
