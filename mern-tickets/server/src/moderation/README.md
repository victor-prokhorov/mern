...
Both the stored term and the incoming text are normalized through the exact same pipeline before comparison. That symmetry is necessary but it is *not* sufficient to make `collapseRepeats` safe, and the seed data demonstrates why: the seeded substring term `ass` normalizes to `as`, and any word containing `as` after collapsing now matches it. `password` collapses to `pasword` and matches; so do `case`, `glass`, `classic`. The app's own demo ticket, "Password reset link is broken.", comes back `flagged: true` for this reason. Collapsing repeats shortens the *term* as well as the input, and every character removed from a short substring term widens its blast radius. See "What this toy skips."
...
- **Word boundaries and the Scunthorpe problem**: substring matching flags innocent words that merely contain a blocked string. The name comes from an April 1996 incident in which AOL's profanity filter refused registrations from residents of Scunthorpe, Lincolnshire; the earliest widely circulated technical write-up is Clive Feather's item in RISKS Digest 18.07, 25 April 1996. `matchType: 'word'` requires the entire normalized token to equal the entire normalized term, which is why `scan('I grew up in Scunthorpe', [{ term: 'cunt', matchType: 'word' }])` returns no match while the same rule with `matchType: 'substring'` does. Both halves of that sentence were run against this code, not assumed.
- **False positives as a product cost**: an over-broad match doesn't just look silly, it makes a real reporter's real ticket disappear or bounce, which is why `matchType` is a per-term choice, not a global setting. This codebase is its own cautionary example — see the `collapseRepeats` note above.
...
- Default `matchType` choices to `word`, not `substring` — substring matching should be an opt-in for specific short strings a reviewer has actually decided are unsafe as any substring. The shorter the term, the higher the bar: a three-character substring term is almost always a mistake.
...
- **`collapseRepeats` combined with short substring terms is over-broad, and the README used to claim otherwise.** Because the pipeline is applied to stored terms too, the seeded term `ass` becomes the two-character string `as`, which then matches inside `password`, `case`, `glass` and `classic`. This is the Scunthorpe problem reappearing *after* the normalization that was supposed to be principled, and it is a design consequence rather than a typo: collapsing runs is only safe for terms long enough that losing a doubled letter does not make them generic. A production pipeline would either apply collapse only to the input side while indexing every collapsed variant of the term, or set a minimum length for substring terms, or both.
...
Now the interesting one, which shows the `collapseRepeats` problem rather than telling you about it:
...
const terms = [{ term: "ass", severity: "flag", matchType: "substring" }];
for (const s of ["Password reset link is broken.", "the glass broke", "he is an assassin apparently"]) {
  console.log(JSON.stringify(s), "->", JSON.stringify(normalize(s)), scan(s, terms, ALLOWLIST).map((t) => t.term));
}'
```
