...
A support-ticket domain: tickets move through a fixed lifecycle, every mutation is written to an append-only audit log, and due dates are derived automatically from priority. This is the foundation everything else in this app wires into: authorization, keyword blocking, throttling, the hook pipeline, optimistic concurrency on writes, the circuit breaker around outbound notification, and the observability layer.

## How it works here

`POST /api/tickets` (`src/routes/tickets.js:8`) calls `tickets.create` (`src/services/tickets.js:35`), which looks up the reporter, stamps `status: 'open'`, computes `dueAt` from priority via `dueAtFor` (`src/services/tickets.js:23`), and writes a `created` `TicketEvent` (`src/services/tickets.js:54`).

`PATCH /api/tickets/:id/status` (`src/routes/tickets.js:11`) calls `transitionStatus` (`src/services/tickets.js:88`), which looks up the ticket's current status, consults the `TRANSITIONS` map (`src/services/tickets.js:13`) for the allowed next states, rejects with 400 `invalid status transition` if `status` is not in that list, otherwise saves the new status and writes a `status_changed` event with `from`/`to`.

Both `PATCH` routes also demand an `If-Match` header carrying the ticket's current `version` — the save goes through `casWriteOrConflict` (`src/services/tickets.js:79-86`), which answers `428` when the header is absent and `412` when the version is stale, so a status change is never a blind write over someone else's. That is a whole feature of its own; [`../concurrency/README.md`](../concurrency/README.md) is where it is explained. It matters here because it changes the shape of every mutation this module describes: the ordering is authorize, then validate the transition against `TRANSITIONS`, then compare-and-set on `version`, then append the event.

`PATCH /api/tickets/:id/assignee` and `POST /api/tickets/:id/comments` follow the same shape: load the ticket, perform the mutation, append a `TicketEvent` (`assignee_changed`, `commented`). Comment creation does not take an `If-Match`, because appending a comment is not a read-modify-write of shared state — nothing about it can be lost to a concurrent update.

In every one of those cases the mutation and the event are two separate writes to two separate collections, with nothing tying them together (`src/services/tickets.js:94-95`, `:107-108`, `:54`, `:125`). MongoDB guarantees atomicity for a single document; it does not give you atomicity across two documents in two collections without an explicit multi-document transaction. A crash between the two writes therefore leaves a mutated ticket with no event recording the mutation — the exact hole the "Standard practice" section is warning about. See "What this toy skips."

`GET /api/tickets/:id` (`src/routes/tickets.js:10`, `src/services/tickets.js:69`) returns the ticket alongside its full comment list and its full event list in one response, so a client can render a timeline without three round trips. Every response that carries a ticket or comment goes through `viewModeratable` (`src/moderation/view.js:5-9`) first — see [`../moderation/README.md`](../moderation/README.md) for why.
...
- **State machine**: the ticket's `status` field only ever moves along edges listed in one exported map (`TRANSITIONS`), never through scattered `if (status === ...)` checks. The map is the single source of truth for what is legal. Note what the graph actually says: `open → triaged → in_progress → resolved`, and from `resolved` either `closed` or back to `open` (a reopen), with `closed` the only state that has no outgoing edges. That terminality holds for everyone, including admins: the policy layer's `closed-ticket-guard` deny leaves admins exempt, but the exemption is unreachable — `TRANSITIONS` is consulted after `authorize`, and `closed` has no outgoing edges, so an admin's transition of a closed ticket passes authorization and still gets 400 `invalid status transition`. Two layers claiming authority over one rule and disagreeing is exactly the two-sources-of-truth drift this bullet warns about, and this pair is the repo's own live instance of it (see [`../policy/README.md`](../policy/README.md)). Be precise about where enforcement lives — the schema's `enum` (`src/models/ticket.js:7`) constrains the *set of states* a ticket may be in, but it says nothing about *edges*; only `transitionStatus` enforces the graph. A direct `Ticket.updateOne({ status: 'closed' })` from anywhere else would pass validation and jump the ticket straight from `open` to `closed`.
...
Two things in that second response are worth a second look. `dueAt` is four hours out because the priority is `urgent`. And `moderation` comes back `{"flagged":false}` — which is only interesting because it used to come back `{"flagged":true}` for that same harmless sentence: the seed once carried a three-character substring term that normalization shortened to two, and `password` normalizes to `pasword`. That is fixed, and the fix is the most useful thing in [`../moderation/README.md`](../moderation/README.md) — see "Recall versus precision" there.
...
Elsewhere in this repo, in the order they build on this module: [`../policy/README.md`](../policy/README.md) decides whether a caller may do any of the above; [`../throttle/README.md`](../throttle/README.md) limits how fast; [`../hooks/README.md`](../hooks/README.md) is the pipeline that runs between the throttle and the write; [`../moderation/README.md`](../moderation/README.md) is the handler that decides whether the body is saved at all; [`../concurrency/README.md`](../concurrency/README.md) is why a status change needs an `If-Match`; [`../circuitBreaker/README.md`](../circuitBreaker/README.md) wraps the webhook `create` and `transitionStatus` fire on the way out; [`../observability/README.md`](../observability/README.md) is what makes any of it visible in production. Outside this app, [`../../../../mern-shop/server/src/passwordReset/README.md`](../../../../mern-shop/server/src/passwordReset/README.md) is the closest thing here to a real credential flow, and [`../../../../mern-movies/server/src/notifications/README.md`](../../../../mern-movies/server/src/notifications/README.md) covers the fan-out-on-a-domain-event problem this app deliberately does not have.
