...
Every ticket carries an explicit `version: { type: Number, required: true, default: 1 }` (`src/models/ticket.js:13`). Mongoose already keeps its own `__v` on every document, primarily for array-subdocument bookkeeping, and it does have an opt-in enforcement mode — the `optimisticConcurrency` schema option makes `document.save()` throw a `VersionError` if `__v` no longer matches what was loaded. That option only wraps `.save()`, though; it does nothing for `updateOne` or `findOneAndUpdate` — which is exactly the atomic, single-round-trip shape this feature needs (see "compare-and-swap as one operation" below), and exactly the case `optimisticConcurrency` does not cover. So even opting into it would not have given this feature what it needs, and the mismatch is a sharper version of the same point: an explicit `version` field this app reads and writes itself makes the mechanism something you can point at in the code, not a Mongoose behavior you have to know applies to one write path and not the other.

`GET /api/tickets/:id` (`src/controllers/tickets.js:16-18`) and every response that hands back a ticket after a write (`create` at `src/controllers/tickets.js:5-9`, `updateStatus` at `:26-30`, `updateAssignee` at `:32-36`) set a strong `ETag` header via `formatETag(ticket.version)` (`src/concurrency/etag.js:3-5`), which renders `version` as `"3"` — quoted, no `W/` prefix, because this is a byte-exact validator over one field, not a cache validator tolerant of insignificant differences.

A mutating request sends that value back as `If-Match`. `readIfMatch` (`src/concurrency/etag.js:7-12`) parses the header into one of three shapes: `{ status: 'missing' }` when the header is absent, `{ status: 'malformed' }` when it does not match `^"(\d+)"$`, or `{ status: 'ok', version }` when it does. The controller (`src/controllers/tickets.js:27`, `:33`) does only this parsing — it never decides what to do about a missing or malformed header itself. That decision is made after authorization, inside the service, by `casWriteOrConflict` (`src/services/tickets.js:79-86`): a missing `If-Match` throws `PreconditionRequiredError` (428), a malformed one throws `BadRequestError` (400), and only a well-formed one reaches the actual write.

The write is `tickets.updateIfVersionMatches(ticket._id, ifMatch.version, update)` (`src/repositories/tickets.js:23-25`), one `findOneAndUpdate({ _id, version: expectedVersion }, { $set: update, $inc: { version: 1 } }, { returnDocument: 'after' })`. The filter and the update happen inside MongoDB as a single operation: the query "does a document with this id and this exact version still exist" and the mutation "change these fields and bump the version" are not two round trips with a gap between them — they are one atomic command. If a concurrent writer already changed `version` in between, the filter matches nothing, MongoDB does nothing, and `findOneAndUpdate` returns `null`, which is the whole signal `casWriteOrConflict` needs: no exception to catch, no partial state to reconcile, just a falsy return value. On a `null`, `casWriteOrConflict` re-reads the ticket (`requireTicket`, `src/services/tickets.js:84`) and throws `PreconditionFailedError('ticket has been modified since the expected version', current.version, viewModeratable(current, subject))` (`src/services/tickets.js:85`, class at `src/middleware/error.js:37-44`), which the shared error handler turns into `412 { error, version, ticket }` (`src/middleware/error.js:61-63`) — the caller gets back exactly what changed, not just a bare failure, and through the same role-based moderation redaction every read path uses (`viewModeratable`, `src/moderation/view.js:5-9` — see [`../moderation/README.md`](../moderation/README.md)), not a raw document.

`transitionStatus` (`src/services/tickets.js:88-98`) and `assign` (`src/services/tickets.js:100-110`) both call `casWriteOrConflict` for their actual write, after `requireTicket` and `authorize` have already run. That ordering is deliberate and is why `test/policy.test.js`'s existing 403 test for an agent transitioning another team's ticket needed no change when this feature landed: authorization and transition-legality are decided against the freshly loaded ticket first, and only a request that clears both ever reaches the precondition check. A forbidden or structurally-invalid request never learns whether it also forgot `If-Match`.

The audit event records which version the write produced, not just that a write happened: `ticketEvents.create({ ..., version: updated.version })` (`src/services/tickets.js:95`, `:108`; field added at `src/models/ticketEvent.js:9`). Given a `status_changed` event with `version: 4`, you know without cross-referencing anything else that this write moved the ticket to version 4 — which write won a race is legible from the audit log alone.
...
- A strong validator that actually varies with representation. The ETag is derived from `version` alone (`formatETag`, `src/concurrency/etag.js:3-5`), but `GET /api/tickets/:id` returns a different JSON representation depending on the caller's role — a reporter's response has `moderation` redacted to `{ flagged }` via `viewModeratable` (`src/moderation/view.js`), an agent's or admin's does not. Two byte-different representations of the same version therefore share one strong validator, which is exactly what a strong ETag is supposed to rule out (see "ETags, `If-Match`, and the two status codes" above). It is harmless here only because `If-Match` is only ever checked against the stored `version` field, never against a hash of the response body itself, so the mismatch never actually causes a wrong conditional match — but it is a real inaccuracy in what the ETag claims to identify, not just a cosmetic one.
...
- [Mongoose docs, `optimisticConcurrency`](https://mongoosejs.com/docs/guide.html#optimisticConcurrency) — Mongoose's own built-in version check, and why it does not cover this app's write path: it only wraps `document.save()`, never `updateOne`/`findOneAndUpdate`.
...
- [Preguiça, Baquero, Shapiro, *Conflict-free Replicated Data Types (CRDTs)*, arXiv:1805.06358 (2018)](https://arxiv.org/abs/1805.06358) — the short encyclopedia-entry overview, and the one to read first. What the next step looks like once whole-document rejection is too coarse: data types designed so concurrent writes merge instead of one being discarded. The original result is Shapiro, Preguiça, Baquero and Zawirski's [*Conflict-free Replicated Data Types*, SSS 2011 / INRIA RR-7687](https://www.lip6.fr/Marc.Shapiro/papers/2011/CRDTs_SSS-2011.pdf), which is where the state-based/operation-based split and the convergence proofs actually live.
