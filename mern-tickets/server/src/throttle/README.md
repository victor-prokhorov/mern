...
- A cross-reference to a `mern-shop/server/src/rateLimit/README.md` companion doc, per the original plan for this feature — that module is landing in `mern-shop` shortly rather than existing yet, so the link isn't live at the time of writing. The distinction between rate limiting and throttling is explained inline above in the meantime; once that doc merges, this README should link it directly instead of restating the comparison.
