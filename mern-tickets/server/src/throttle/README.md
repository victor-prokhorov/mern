...
The write in `consume` is a compare-and-swap: `buckets.updateIfUnchanged` (`src/repositories/tokenBuckets.js:12`) matches on `_id`, `updatedAt`, and `tokens` together as they were just read, so if a concurrent request already modified the same bucket, the update matches nothing, returns `null`, and `consume` retries the whole read-compute-write cycle (`src/throttle/tokenBucket.js:32-43`) from the fresh state — this is what "atomic" means here: no two concurrent requests can both believe they spent the same token.
...
- A cross-reference to a `mern-shop/server/src/rateLimit/README.md` companion doc, per the original plan for this feature — no such module exists in `mern-shop` at the time of writing (`mern-shop/server/src/rateLimit` was not found in the repository), so there is nothing there to link to. The distinction between rate limiting and throttling is instead explained inline above.
