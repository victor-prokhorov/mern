...
Every mutating and reading ticket service function calls `authorize` before touching data: `create` (`src/services/tickets.js:32`), `get` (`src/services/tickets.js:62`), `transitionStatus` (`src/services/tickets.js:72`), `assign` (`src/services/tickets.js:84`), `addComment` (`src/services/tickets.js:97`). Controllers never check a role themselves — they pass `req.subject` (set by `identify`, `src/middleware/identify.js`) straight through to the service, which builds the `{ subject, action, resource, context }` request and asks the engine.
...
`GET /api/tickets` (list) does not call `authorize` per row. Instead `list` (`src/services/tickets.js:50`) scopes the database query itself: reporters only ever see their own tickets, agents only their team's. This is a pragmatic stand-in for row-level authorization on a collection endpoint — see "What this toy skips" for the gap it leaves.
