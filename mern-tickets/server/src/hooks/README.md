...
`register` (`src/hooks/registry.js:5`) appends a handler to an array keyed by event name in a module-level `Map` (`src/hooks/registry.js:1`) — registration order is preserved because it is just array push order. `run` (`src/hooks/registry.js:33`) walks that array for the given event, threading a `current` payload through each handler: a handler's return value is inspected for its `action` field, and `continue` (or a thrown error) moves to the next handler with `current` unchanged; `reject` returns immediately from `run` with `{ action: 'reject', reason }`, so no handler after it ever runs; `transform` replaces `current` with `result.payload`, but only when `result.payload` is actually present — a `transform` result with no `payload` field is treated as a no-op rather than wiping the pipeline's payload to `undefined` (`src/hooks/registry.js:44-46`). If every handler continues, `run` returns `{ action: 'continue', payload: current }` with whatever the last transform left behind.

Each handler call is wrapped in `withTimeout` (`src/hooks/registry.js:16`), which races the handler against a fixed budget (`HANDLER_TIMEOUT_MS`, `src/hooks/registry.js:3`), clearing the timer as soon as the handler settles either way and `unref`-ing it so a pending one never blocks process exit. A timeout is treated as a thrown error. Both a genuine throw and a timeout are caught in the same place (`src/hooks/registry.js:38-43`): the error is logged and the loop moves on to the next handler as if this one had returned `continue`. This is fail-open by construction — a broken handler degrades moderation, it does not block the request.
