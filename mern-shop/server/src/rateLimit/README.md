...
7. `app.js:14-16` wires three limiter instances: 5 requests/minute keyed by IP for login, 5/minute keyed by the submitted email for login, and 3/hour keyed by email for `forgot-password`. `app.js:21-22` applies them as route-scoped middleware before the real routers are mounted.
