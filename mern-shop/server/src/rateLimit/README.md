...
7. `app.js:14-18` wires five limiter instances: 5 requests/minute keyed by IP for login, 5/minute keyed by the submitted email for login, 3/hour keyed by email for `forgot-password`, 10/hour keyed by IP for `reset-password`, and 20/hour keyed by IP shared across `refresh` and `logout`. `app.js:23-26` applies them as route-scoped middleware before the real routers are mounted. The last three limits are deliberately looser and IP-only rather than credential-keyed: the values being checked there are 256-bit random tokens, so the limiter is resource protection (stop one IP from hammering the endpoint with junk requests) rather than credential protection (there is no realistic brute force of a 256-bit token to defend against). See [`../session/README.md`](../session/README.md) for what `refresh` and `logout` do with those tokens.
...
- The email-keyed limiters key on `req.body.email`, so every request whose body is missing or unparseable (nothing for `express.json` to produce) lands in one shared anonymous bucket keyed `login:email:` — five malformed POSTs per window exhaust it for all such requests at once, and none of them count against any real address.
...
Finally, watch the fixed-window boundary burst — the weakness this algorithm has by construction. This one needs a genuinely idle server: the IP-keyed counter is shared by every login request from your machine, so anything you sent in the current minute (including the two demos above) is already charged against the five. Wait out a full minute of silence first, then start just before the top of the next one so the run straddles a window boundary:
