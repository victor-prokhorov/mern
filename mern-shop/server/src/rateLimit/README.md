...
So a spec-conformant version of what this middleware emits for the login limiter would be roughly `RateLimit-Policy: "login";q=5;w=60` and `RateLimit: "login";r=4;t=21`, not three separate integer-valued fields. **This code is not conformant with the current draft and this README does not claim it is.** It emits the older, wider-deployed spelling on purpose: the draft syntax has now changed more than once, most existing clients and middleware still read the `-05` names, and a teaching demo pinned to a moving draft would be wrong in a different way every year. Anything you deploy for real should pick deliberately, and know which of the two it picked.

There is a second, sharper caveat, and it is a defect rather than a deliberate tradeoff. Login runs **two** limiters (`app.js:22`), and each one overwrites the previous one's headers, so the response describes only the last limiter to run — the email-keyed one — not whichever limiter is actually closest to blocking you. You can watch it happen: in a fresh window, send three login attempts as `a@shop.test`, then one as `b@shop.test`. The IP-keyed counter is now at 4 of 5, but the response says `RateLimit-Remaining: 4`, because the email-keyed counter for `b@shop.test` is at 1 of 5. A client that trusts the header is told it has four requests left when it has one. A correct implementation reports the *minimum* remaining budget across all limiters that ran, and a `RateLimit-Policy`-style field can name which policy each figure belongs to — which is precisely the problem the current draft's partition keys and named policies exist to solve.
...
- Report the binding limit, not the last one computed — when several limiters stack on one route, the numbers a client sees must reflect whichever is closest to rejecting it (see the demonstration above), or the headers actively mislead the well-behaved clients they exist to help.
...
- Headers describe the last limiter that ran rather than the binding one, and use an obsolete draft's field syntax — both covered above, both defects rather than simplifications.
...
Now reproduce the header bug. In a fresh window, spend three attempts on one email and then send one on another:
...
The response reports `RateLimit-Remaining: 4`, because the email-keyed limiter ran last and overwrote the IP-keyed limiter's headers. The IP-keyed counter is actually at 4 of 5, so one more request from this address is blocked no matter which email it carries.
