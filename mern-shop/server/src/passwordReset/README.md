...
6. On success it bcrypt-hashes the new password at cost 10 (`services/passwordReset.js:36`), writes it via `users.updatePasswordHash` (`repositories/users.js`), revokes every session belonging to that user (`sessions.revokeAllForUser`, `repositories/sessions.js`) in the same step, and invalidates every other outstanding token belonging to that user (`repositories/passwordResets.js:15-17`) so an older, still-live token from a previous request can't be replayed after a newer flow already changed the password.
...
- No TTL index on `PasswordReset` — used and expired token rows are excluded by every query (`consumeToken` filters on `usedAt`/`expiresAt`) but never physically deleted, so the collection grows forever. The sibling models get this right (`RateLimit` and `IdempotencyKey` both carry `expiresAt` with `expires: 0`); this one just never got the same index.
