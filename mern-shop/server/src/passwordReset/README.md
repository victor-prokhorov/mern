...
- Rate limit the forgot-password endpoint per email — otherwise it becomes a script for testing which emails are registered, and a way to spam a victim's inbox (see `server/src/rateLimit/README.md`). This app applies 3 requests per hour per email to `/forgot-password` (`app.js:16`, `app.js:22`); `/reset-password` carries no rate limit of its own on this branch, which is a real gap — see below.
...
- `/forgot-password` is rate limited (3/hour/email, from Task 2), but `/reset-password` is not rate limited at all on this branch — an attacker who obtains or guesses a token-shaped string can hammer that endpoint without being slowed down. It should carry its own limiter, keyed at minimum by IP.
