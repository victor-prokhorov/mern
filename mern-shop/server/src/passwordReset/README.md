...
1. `POST /api/auth/forgot-password { email }` is routed in `routes/auth.js:8` to `controllers/passwordReset.js`'s `forgotPassword`, which calls `services/passwordReset.js:16` `forgotPassword(email)`.
