...
- Invalidate other sessions when the password changes. **This app does not, and that is now a real gap rather than a missing feature.** It used to be neither: before [`../session/README.md`](../session/README.md)'s feature landed, login just returned the user document and there was nothing to revoke. There is now — `Session` rows keyed by `familyId`, with a `sessions.revokeFamily` already written and already used by logout and by refresh-token reuse detection — and `services/passwordReset.js`'s `resetPassword` still never calls it. So an attacker who logged in with the stolen password before the reset keeps a valid refresh token afterwards and can rotate it indefinitely; the reset changed the credential they no longer need. Closing it would mean revoking every family belonging to `record.user` inside `resetPassword`, in the same step that writes the new hash.
- Never log the raw token, in any environment, for any reason — a reset token is a bearer credential exactly as sensitive as the password it resets, and stdout/log aggregators are rarely treated with the same access controls as the primary database. This app's own console log of the raw token (`services/passwordReset.js:24`) is only acceptable because it is gated behind an opt-in flag meant strictly for local development and this test suite; it should never be reachable in a deployed environment (see the `EXPOSE_RESET_TOKEN` note above).
...
- The reset does not revoke sessions, even though the app has had a revocable session store since [`../session/README.md`](../session/README.md) landed. This is the one gap in this file that is a live bug rather than a scope decision: every other item below is something a toy is entitled to skip, and this one is something the code is now equipped to do and does not.
...
- [`../session/README.md`](../session/README.md) — the session store this flow ought to be revoking, and the same hashed-token-plus-atomic-single-use-consumption shape one level over, applied to refresh tokens.
