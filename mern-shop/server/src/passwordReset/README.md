...
1. `POST /api/auth/forgot-password { email }` is routed in `routes/auth.js:8` to `controllers/passwordReset.js`'s `forgotPassword`, which calls `services/passwordReset.js:15` `forgotPassword(email)`.
...
6. On success it bcrypt-hashes the new password at cost 10 (`services/passwordReset.js:35`), writes it via `users.updatePasswordHash` (`repositories/users.js`), and invalidates every other outstanding token belonging to that user (`repositories/passwordResets.js:15-17`) so an older, still-live token from a previous request can't be replayed after a newer flow already changed the password.

**Where this app is behind current guidance, stated plainly.** Two of its constants are no longer defensible defaults, and a reader should not take them as models:

- **Minimum password length 8** (`services/passwordReset.js:30`). NIST SP 800-63B-4 (final, 26 August 2025) §3.1.1.2 says verifiers "**SHALL** require passwords that are used as a single-factor authentication mechanism to be a minimum of 15 characters in length", and permits eight only for passwords "used as part of multi-factor authentication processes". This app has no second factor, so 8 is the MFA-only floor applied to a single-factor system — roughly half the required length. It also does no screening against breached-password lists, which the same section makes a **SHALL** ("verifiers **SHALL** compare the prospective secret against a blocklist that contains known commonly used, expected, or compromised passwords"). Length is the cheaper of the two to fix and the less effective; the blocklist is what actually stops the passwords attackers try first.
- **bcrypt cost 10** (`services/passwordReset.js:35`). This is still *inside* current guidance but sitting on its floor: OWASP's Password Storage Cheat Sheet says the work factor "should be as large as verification server performance will allow, with a minimum of 10". Same sheet, same page, puts bcrypt third in preference order — Argon2id first (m=19456 KiB, t=2, p=1 as the minimum recommended configuration), scrypt second (N=2^17, r=8, p=1), bcrypt for legacy systems, PBKDF2 when FIPS compliance forces it. What Argon2id buys over bcrypt is memory hardness: bcrypt's 4 KiB working set fits trivially in GPU and ASIC silicon, so an attacker parallelises cheaply; Argon2id's tunable memory cost makes each guess expensive in RAM as well as cycles, which is much harder to buy in bulk. Cost 10 in 2026 is a defensible floor for an existing system that already uses bcrypt everywhere, and a poor choice for a new one.
...
- Screen the new password against a breached-password corpus at the moment it is set, and enforce a length floor that matches your actual authentication design (15 characters single-factor, 8 only alongside a second factor) — this endpoint is one of the two places where a password enters the system, so a policy that is only enforced at registration is not enforced.
...
- No minimum-length policy that matches a single-factor design, and no breached-password screening — both covered above, both places where this app is behind NIST SP 800-63B-4 rather than merely simplified.
...
curl -s -X POST http://localhost:5000/api/auth/reset-password \
  -H 'Content-Type: application/json' \
  -d '{"token":"deadbeef","password":"newpassword1"}'
```
...
Finally, watch the length check reject a password that current NIST guidance would also reject, and accept one it would not:

```bash
curl -s -X POST http://localhost:5000/api/auth/reset-password \
  -H 'Content-Type: application/json' \
  -d '{"token":"whatever","password":"short"}'
```

This returns `password must be at least 8 characters` — note it is checked before the token is even looked up, so it answers for an invalid token too. An eight-character password is accepted here and would fail NIST SP 800-63B-4's single-factor minimum of 15; that gap is the app's, not the reader's, and is documented above rather than papered over.
...
- [OWASP Session Management Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html) — the section on renewing the session ID after any privilege change is the missing step 7 of this flow. Worth reading precisely because this app has no sessions to invalidate and therefore cannot show you what it looks like.
