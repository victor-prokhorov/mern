...
1. **Per-user block** — two fields directly on the account: `User.blockedAt` and `User.blockReason` (`models/user.js:8-9`). Set via `services/blocks.js:53-60` (`blockUser` / `unblockUser`), which call `repositories/blocks.js:20-26` and write straight to the `User` document by its `_id`. This targets one already-known account.
2. **Pattern blocklist** — a separate `BlockEntry` collection (`models/blockEntry.js`) of `{ type: 'email' | 'domain', value, reason, createdBy, createdAt }` records, managed through `POST /api/blocks` and `DELETE /api/blocks/:id` (`routes/blocks.js:6-7`, `controllers/blocks.js:17-28`). This targets an *identifier*, not an account, so it also catches accounts that don't exist yet.
...
3. **Normalization** lives in one place, `services/blocks.js:8-17` (`normalizeEmail`), and both write and read paths call it: `createBlock` normalizes the `value` before storing an email-type entry (`services/blocks.js:41`), and `isBlockedEmail` normalizes the incoming address before comparing (`services/blocks.js:28`). It lowercases, trims, strips anything after the first `+` in the local part, and — only for `gmail.com` / `googlemail.com` — strips dots from the local part, so `Demo+spam@Shop.test` normalizes to `demo@shop.test` and matches a stored block on `demo@shop.test`.
...
4. **Enforcement points check account identity, never a checkout-supplied value**: `services/auth.js:24` checks `user.blockedAt` and `isBlockedEmail(user.email)` — the account's *own*, database-backed email — after the password already matched, and — if blocked — throws the exact same `UnauthorizedError('invalid credentials')` (401) as a wrong password (`services/auth.js:23`), so a login attempt against a blocked account is indistinguishable from a login attempt with the wrong password. `services/orders.js:15` runs the identical check — `user.blockedAt` or `isBlockedEmail(user.email)` — before the cart is ever touched, and throws `ForbiddenError('account is not available')` (403, added to `middleware/error.js`) if either is true, so a refused order leaves the cart exactly as it was. Login's 401 and order placement's 403 differ in status and text quite deliberately: login folds a blocked account into the same response as a wrong password because there is a password-shaped response to hide inside; order placement has no such response to borrow, so it uses its own distinct, still-uninformative message instead. The checkout-supplied `customer.email` is deliberately **not** checked here — see the note below on where that value is actually evaluated. Two further enforcement points close the live-session gap: `blockUser` (`services/blocks.js`) revokes every session belonging to the account (`sessions.revokeAllForUser`) in the same step that sets `blockedAt`, so a block bites the sessions that already exist and not just the next login; and the refresh flow (`services/auth.js`, `refresh`) re-loads the account on every rotation and — when `blockedAt` is set or the account's own email matches the pattern blocklist — revokes the family and throws the same `401 invalid refresh token` as any other bad token. The refresh-time check is what catches a *pattern* block created after login, which revokes nothing by itself; without both, a blocked user's already-issued refresh token kept rotating for up to 30 days, because the only thing a block ended was the ability to log in again. This is the same recipe password reset applies one directory over ([`../passwordReset/README.md`](../passwordReset/README.md)): kill the sessions in the same step as the state change, and re-check current account state wherever a new token is minted.
5. **Admin surface**: `controllers/blocks.js:11-15` (`requireAdmin`) compares the `x-admin-token` request header against `process.env.ADMIN_TOKEN` using `crypto.timingSafeEqual` (`safeEqual`, `controllers/blocks.js:5-9`) rather than `!==`, so a wrong guess cannot be distinguished by response timing — the shared secret is already weak on its own, and there's no reason to add a timing side channel on top of it. `safeEqual` compares SHA-256 digests of the two inputs rather than the raw bytes: `timingSafeEqual` demands equal-length buffers, and hashing both sides to a fixed 32 bytes satisfies that for any input pair, so even a guess of the wrong *length* costs the full constant-time comparison instead of leaking the secret's length through an early return. It throws `UnauthorizedError` on a missing, wrong, or unconfigured token. `POST /api/blocks` creates a `BlockEntry` (`controllers/blocks.js:17-22`) with `createdBy` taken from an `x-admin-name` header (defaulting to `'admin'`) and `createdAt` defaulted by the schema — that's the audit record. `DELETE /api/blocks/:id` removes an entry (`controllers/blocks.js:24-28`), and its own request is itself only possible with a valid admin token, so unblocking is exactly as gated as blocking.

**Why the account gate ignores the checkout email**: an earlier version of this code checked `isBlockedEmail(customer.email)` at the order-placement gate instead of `user.email`. That was a real bypass — a blocked account could place an order freely just by typing a different email into the checkout form, since nothing about the *account* was ever consulted. It also made the fraud scorer's `BLOCKED_RECIPIENT` signal permanently dead: any blocked-domain checkout email was already rejected earlier (with the wrong message) before scoring ever ran. The fix gives each layer exactly one job: this blocklist enforces identity (is *this account* blocked, by itself or by a pattern matching its own email) at a hard, unconditional gate; the checkout-supplied email is evaluated only by the fraud scorer's `BLOCKED_RECIPIENT` signal (see [`../fraud/README.md`](../fraud/README.md)), which weighs it alongside other signals instead of hard-gating on it. A checkout email on a blocked domain is still refused end to end — just via `403 order could not be completed` from the fraud layer, not `403 account is not available` from this one.
...
- Gate the admin surface with real authorization, not a single shared secret — see [`mern-tickets/server/src/policy/README.md`](../../../../mern-tickets/server/src/policy/README.md) for what an actual authorization system for admin actions looks like; a header secret has none of the properties (per-actor identity, revocation, least privilege) that a real admin control needs, even once the comparison itself is constant-time. `crypto.timingSafeEqual` also refuses buffers of different lengths outright, and an earlier version of `safeEqual` handled that by returning early on a length mismatch — leaking the secret's length through timing and excusing it as unavoidable. It is avoidable, and the standard fix is what `safeEqual` (`controllers/blocks.js:5-9`) now does: hash both inputs to fixed-length SHA-256 digests first, then `timingSafeEqual` the digests — every comparison is 32 bytes against 32 bytes regardless of what the caller sent, so neither the content nor the length of the expected token shapes the response time.
...
The first order is refused with `403 { "error": "order could not be completed" }` — from the fraud scorer's `BLOCKED_RECIPIENT` signal, not from this layer — and because a denied order leaves the cart untouched, the second request finds the same cart still full and is created. One extra label is the entire evasion.
...
- [Node.js crypto: `timingSafeEqual`](https://nodejs.org/api/crypto.html#cryptotimingsafeequala-b) — the exact contract of the comparison `requireAdmin` relies on, including the equal-length requirement that is the reason `safeEqual` compares SHA-256 digests of the inputs rather than the raw bytes.
