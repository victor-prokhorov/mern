...
1. **Per-user block** — two fields directly on the account: `User.blockedAt` and `User.blockReason` (`models/user.js:8-9`). Set via `services/blocks.js:52-58` (`blockUser` / `unblockUser`), which call `repositories/blocks.js:20-25` and write straight to the `User` document by its `_id`. This targets one already-known account.
